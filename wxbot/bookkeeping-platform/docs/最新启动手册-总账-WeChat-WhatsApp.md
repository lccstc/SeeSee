...
```bash
BOOKKEEPING_SYNC_TOKEN="test-token-123456" \
BOOKKEEPING_MASTER_USERS="+852389225210" \
python3 "/Users/lcc/SeeSee/wxbot/bookkeeping-platform/reporting_server.py" \
  --host "0.0.0.0" \
  --port 8765 \
  --db "/Users/lcc/SeeSee/wxbot/bookkeeping-platform/data/bookkeeping-test.db"
```

### 这条命令每一部分是什么意思

- `BOOKKEEPING_SYNC_TOKEN`
...
```json
{
  ...
  "log_level": "INFO",
  "language": "cn",
  "db_path": "C:\\wxbot\\bookkeeping-platform\\data\\bookkeeping.db",
...
- `db_path`
给 WeChat 适配器本地控制面和身份解析用。
现在正式总账事实源不是这里，而是远端总账中心。
...
- `core_api.token`
必须和总账启动时的 `BOOKKEEPING_SYNC_TOKEN` 一模一样。
...
对总账中心来说，真正主数据库是你启动 `reporting_server.py` 时传的 `--db`。

WeChat 的 `config.wechat.json` 里虽然也有 `db_path`，但在你现在推荐的远端模式里，它不是正式统一事实源。
