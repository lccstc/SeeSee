...
class CommandHandler:
    def __init__(self, db, platform_api, master_users: list[str], export_dir: str | Path) -> None:
        self.db = db
    ...
    def handle_command(self, *, platform: str, group_key: str, chat_id: str, chat_name: str, sender_id: str, sender_name: str, command_text: str) -> None:
    ...
    def handle_balance(self, group_key: str, chat_id: str) -> None:
        ...
        if ngn_rate is not None and is_group2:
            msg += f"\nNGN: {ngn_rate:g}"
        self.platform_api.send_text(chat_id, msg.rstrip())
    ...
    def handle_history(self, group_key: str, chat_id: str, args: str) -> None:
        ...
        if not rows:
            self.platform_api.send_text(chat_id, "No transactions")
            return
        ...
        for row in reversed(rows):
            ...
            if row["category"] == "rmb":
                ...
                lines.append(f"{time_text} | {input_sign}{row['amount']:g}{row['category'].upper()} x{row['rate']:g} = {rmb_sign}{abs(row['rmb_value']):.2f}")
        self.platform_api.send_text(chat_id, "\n".join(lines))

    def handle_undo(self, group_key: str, chat_id: str, sender_id: str) -> None:
        if not self.can_manage(sender_id):
            self.platform_api.send_text(chat_id, "Permission denied")
            return
        if self.last_undo_group_key == group_key:
            self.platform_api.send_text(chat_id, "Consecutive undo blocked\nPlease record a new transaction first")
            return
        deleted = self.db.undo_last(group_key)
        if deleted is None:
            self.platform_api.send_text(chat_id, "No transaction to undo")
            return
        self.last_undo_group_key = group_key
        sign = "+" if deleted["rmb_value"] >= 0 else "-"
        self.platform_api.send_text(chat_id, f"Undone: {deleted['raw']} ({sign}{abs(deleted['rmb_value']):.2f})\nTo continue undoing, please record a new transaction first")

    def handle_clear(self, group_key: str, chat_id: str, sender_id: str) -> None:
        if not self.is_master(sender_id):
            self.platform_api.send_text(chat_id, "Admin only")
            return
        count = self.db.clear_group(group_key)
        self.platform_api.send_text(chat_id, f"Cleared {count} transactions")
    ...
    def handle_add_user(self, chat_id: str, sender_id: str, args: str) -> None:
        ...
        user_key = args.strip()
        if not user_key:
            self.platform_api.send_text(chat_id, "Format: /adduser 微信号")
            return
        self.db.add_to_whitelist(user_key, sender_id)
        self.platform_api.send_text(chat_id, f"Added user: {user_key}")
    ...
    def handle_remove_user(self, chat_id: str, sender_id: str, args: str) -> None:
        ...
        user_key = args.strip()
        if not user_key:
            self.platform_api.send_text(chat_id, "Format: /rmuser 微信号")
            return
        if self.is_master(user_key):
            self.platform_api.send_text(chat_id, "Cannot remove admin by /rmuser")
            return
        removed = self.db.remove_from_whitelist(user_key)
        self.platform_api.send_text(chat_id, f"{'Removed user' if removed else 'User not found'}: {user_key}")
    ...
    def handle_add_master(self, chat_id: str, sender_id: str, args: str) -> None:
        ...
        user_key = args.strip()
        if not user_key:
            self.platform_api.send_text(chat_id, "Format: /addmaster 微信号")
            return
        self.db.add_admin(user_key, sender_id)
        self.platform_api.send_text(chat_id, f"Added admin: {user_key}")
    ...
    def handle_remove_master(self, chat_id: str, sender_id: str, args: str) -> None:
        ...
        user_key = args.strip()
        if not user_key:
            self.platform_api.send_text(chat_id, "Format: /rmmaster 微信号")
            return
        if user_key in self.bootstrap_masters:
            self.platform_api.send_text(chat_id, "Cannot remove bootstrap admin")
            return
        removed = self.db.remove_admin(user_key)
        self.platform_api.send_text(chat_id, f"{'Removed admin' if removed else 'Admin not found'}: {user_key}")
    ...
    def handle_export(self, group_key: str, chat_id: str, sender_id: str) -> None:
        ...
        if path is None:
            self.platform_api.send_text(chat_id, "No data to export")
            return
        if not self.platform_api.send_file(chat_id, str(path)):
            self.platform_api.send_text(chat_id, f"Exported file ready: {path}")
    ...
    def handle_all_settle(self, platform: str, chat_id: str, sender_id: str) -> None:
        ...
        if not rows:
            self.platform_api.send_text(chat_id, "No unsettled transactions in any group")
            return

        self.platform_api.send_text(chat_id, f"Starting batch settlement...\n{len(rows)} groups to settle")
        total_groups = 0
        total_txs = 0
        total_rmb = 0.0
        details: list[str] = []
        for row in rows:
            ...
            self.platform_api.send_text(row["chat_id"], f"Happy transaction!\nBalance: {'+' if bal['total'] >= 0 else ''}{bal['total']:.2f}")
    ...
    def handle_settlements(self, group_key: str, chat_id: str, args: str) -> None:
        ...
        if not rows:
            self.platform_api.send_text(chat_id, "No settlement records")
            return
    ...
    def handle_mingxi(self, group_key: str, chat_id: str, args: str) -> None:
        ...
        if cat_data is None or not cat_data["rate_groups"]:
            self.platform_api.send_text(chat_id, f"No transactions for {category.upper()}")
            return
        bal_sign = "+" if bal["total"] >= 0 else ""
    ...
    def handle_set_group(self, platform: str, group_key: str, chat_id: str, chat_name: str, sender_id: str, args: str) -> None:
        ...
        if not args or not args.isdigit() or len(args) != 1:
            self.platform_api.send_text(chat_id, "Format: /set number (0-9)\nEx: /set 1")
            return
        group_num = int(args)
        if not self.db.set_group(platform=platform, group_key=group_key, chat_id=chat_id, chat_name=chat_name, group_num=group_num):
            self.platform_api.send_text(chat_id, "Failed, please retry")
            return
        self.platform_api.send_text(chat_id, f"This group assigned to Group {group_num}\nOnly groups with numbers can use bookkeeping")
    ...
    def handle_diy_send(self, chat_id: str, sender_id: str, args: str) -> None:
        ...
        if len(parts) != 2 or not parts[0].isdigit():
            self.platform_api.send_text(chat_id, "Format: /diy number message\nEx: /diy 1 Hello everyone")
            return
        group_num = int(parts[0])
        message = parts[1].strip()
        groups = self.db.get_groups_by_num(group_num)
        if not groups:
            self.platform_api.send_text(chat_id, f"Group {group_num} has no groups")
            return
        total_groups = len(groups)
        estimated_time = max(1, int((total_groups / 5) * 1.5 + 0.999))
        self.platform_api.send_text(chat_id, f"Starting broadcast...\nGroup: {group_num}\nTarget: {total_groups} groups\nEst. time: {estimated_time}s")
        results: list[tuple[dict, bool]] = []
        for index, group in enumerate(groups, start=1):
            ok = self.platform_api.send_text(group["chat_id"], message)
            results.append((group, ok))
            if index % 10 == 0:
                success = sum(1 for _, sent in results if sent)
                self.platform_api.send_text(chat_id, f"Progress: {index}/{total_groups} (Success: {success})")
    ...
    def handle_ngn(self, chat_id: str, sender_id: str, args: str) -> None:
        ...
        try:
            ...
            self.platform_api.send_text(chat_id, "Format: /ngn number\nEx: /ngn 194.2")
            return
        self.db.set_ngn_rate(str(rate))
        self.platform_api.send_text(chat_id, f"NGN rate updated: {rate:g}")
