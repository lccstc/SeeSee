...
你已经进一步明确了一个关键业务判断：每个供应商群虽然表面格式变化很多，但本质上通常只有有限几种稳定模式，例如整版报价、局部更新、固定 section 组合，以及若干 header + numeric rows 的结构。这意味着项目不应该追求“训练一个万能解析器”，而应该为每个群持续沉淀一份可验证、可回放、可迭代完善的专属解析语法书。新格式第一次可以失败，但失败后必须能被异常池、replay、状态机和工业化修补流程逐步吸收到该群 profile 或跨群共享能力中。

你又确认了一个进入生产阶段前必须补上的现实约束：repair case 的失败日志不能只是一串当前主 agent 才看得懂的上下文记录。随着上下文窗口轮换、主 agent 更替和生产值班场景到来，这些日志必须进一步演化成项目自己的 **Failure Dictionary / Quote Repair Lexicon**。它应该像“新华字典”一样，把常见失败模式、根因、禁止修法、推荐修补层级、已验证 replay fixture、已通过测试和 known-good fix 全部结构化索引起来，让新的主 agent 或 subagent 在没有历史对话上下文的情况下，也能先查词条、按标准流程修、修完再回写修订，而不是每次从零理解系统。
...
- **Group Strategy**: 一群一 profile 是主架构 — 默认先完善该群 profile / section / bootstrap，不先追求全局万能解析
...
- **Repair Workflow**: 异常修补必须受状态机约束 — SubAgent 只能提出群级或系统级修补提案，最终吸收必须经过 replay、validator 和回归验证
- **Knowledge Capture**: repair case 失败日志必须沉淀为结构化故障字典 — 生产阶段的新 agent 不能依赖当前对话上下文，必须能通过项目词典检索已知错误、标准修法和禁用修法
...
| 报价墙不是训练一个万能解析器，而是为每个群沉淀专属解析语法书 | 每个群本质上是有限稳定模式的组合，最安全的修补应先落在群级 profile | — Pending |
| 异常修补必须通过受约束的状态机推进，而不是无限人工补 parser | 新格式第一次可失败，但失败后应形成标准 repair case、有限次尝试、失败升级与工业化沉淀 | — Pending |
| repair case 日志必须进化成项目故障字典，而不是上下文依赖型流水账 | 生产值班的主 agent / subagent 不会天然拥有当前会话上下文，必须先查结构化词条再修补 | — Pending |
| 实验墙阶段允许系统真实更新你本人运营的报价墙，但不自动授予正式生产 authority 或任何下游动作 | 当前旧解析链路已被判定不足以作为生产标准；更合理的做法是在真实流量下验证新系统，但继续把下游自动化关掉 | — Pending |
